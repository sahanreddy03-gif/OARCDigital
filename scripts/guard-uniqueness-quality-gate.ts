...
/**
 * Indexing quality gate tighten (uniqueness overlays batch1+batch2+batch3+batch4+batch5).
 *
...
function main() {
  console.log(
    `guard-uniqueness-quality-gate: batches=${UNIQUENESS_OVERLAY_BATCH1.batch}+${UNIQUENESS_OVERLAY_BATCH2.batch}+${UNIQUENESS_OVERLAY_BATCH3.batch}+${UNIQUENESS_OVERLAY_BATCH4.batch}+${UNIQUENESS_OVERLAY_BATCH5.batch} overlayEntries=${UNIQUENESS_OVERLAY_ALL_ENTRIES.length} GLOBAL_KEEP=${LOCATION_IND_SVC_GLOBAL_KEEP}`,
  );
