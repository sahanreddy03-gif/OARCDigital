...
// Parity gate for the full-reference AI surface: the AUTOGEN-bounded body is
// regenerated from SERVICE_SCHEMAS + PILLAR_SCHEMAS + SERVICE_SCHEMA_EXTRAS.
// We test idempotency — applyLlmsFullTransform(current) === current iff the
// body matches generator output. Content OUTSIDE the markers is preserved
// across runs and intentionally NOT compared.
function checkLlmsFullTxtParity(): Issue[] {
  ...
  if (current !== expected) {
    out.push({
      slug: "_global",
      layer: 4,
      message: `${LLMS_FULL_PATH} AUTOGEN body drifted from SERVICE_SCHEMAS / PILLAR_SCHEMAS / SERVICE_SCHEMA_EXTRAS — run \`npx tsx scripts/generate-llms-full-txt.ts\` and commit`,
    });
  }
  return out;
}
