...
export const SERVICE_SCHEMA_EXTRAS: Record<string, ServiceSchemaExtra> = {
  ...
  "ai-restaurant-voice-host": {
    title: "HOST — Restaurant Voice AI Malta | H360 Voice Host | OARC Digital",
    description: "HOST answers every restaurant call in Malta 24/7 — books tables, handles allergies and events, trains on your menu. OARC shadow-trains the brain; you keep dashboard control.",
    serviceType: "Restaurant Voice AI — Phone Answering & Booking",
    audience: ["Restaurants", "Hospitality venues"],
    areaServed: "Malta",
    offers: [{ name: "HOST Voice Pilot", priceFrom: 890, unitText: "MONTH", description: "Malta restaurant line, menu training, H360 booking sync, staff alerts, dashboard." }],
    features: [{ name: "24/7 inbound call answering" }, { name: "Table booking sync" }, { name: "Allergy and event handling" }, { name: "VIP routing" }, { name: "OARC operator training" }, { name: "Human handoff rules" }],
    faqs: [{ question: "Is HOST the same as H360 Voice Host?", answer: "Yes — HOST is the OARC Digital commercial product. The full H360 cinema page lives at /h360/restaurant-phone-ai-malta. Same brain, same training." }],
  },
  "ai-voice-receptionist": {
    title: "FRONT — AI Receptionist Malta | OARC Digital",
    description: "FRONT is OARC's AI reception — answers every call, routes departments, books meetings, escalates with full context. Shadow-trained on your business, not a generic call centre.",
    serviceType: "AI Receptionist — Voice Answering",
    offers: [{ name: "FRONT Reception", priceFrom: 690, unitText: "MONTH", description: "Business line, routing rules, calendar sync, after-hours paging." }],
    features: [{ name: "24/7 call answering" }, { name: "Department routing" }, { name: "Meeting booking" }, { name: "VIP lines" }, { name: "Escalation with transcript" }],
    faqs: [{ question: "Can FRONT keep our Malta phone number?", answer: "Yes — forward your existing line or provision a new Malta number. Guests dial the same number they always have." }],
  },
  "ai-voice-csr": {
    title: "LINE — AI CSR Malta | Customer Service Voice Agent | OARC Digital",
    description: "LINE handles billing, rescheduling, reminders, and follow-ups on voice and text — like your best CSR, 24/7, with human escalation when needed.",
    serviceType: "AI Customer Service Representative — Voice & Text",
    offers: [{ name: "LINE CSR", priceFrom: 990, unitText: "MONTH", description: "Voice + SMS, ticket integration, escalation rules, weekly tuning." }],
    features: [{ name: "Billing and account inquiries" }, { name: "Rescheduling" }, { name: "Reminder cadences" }, { name: "Happy-call feedback" }, { name: "Sentiment escalation" }],
    faqs: [{ question: "Does LINE replace my support team?", answer: "It removes tier-1 repeat volume so humans handle complex cases. You keep full dashboard control and approval on brain updates." }],
  },
  "ai-voice-dispatcher": {
    title: "ROUTE — AI Dispatcher Malta | Field Service Voice AI | OARC Digital",
    description: "ROUTE qualifies inbound jobs, assigns technicians, manages capacity, and syncs to your dispatch board — built for Malta HVAC, plumbing, and trades.",
    serviceType: "AI Dispatcher — Field Service",
    offers: [{ name: "ROUTE Dispatch", priceFrom: 1290, unitText: "MONTH", description: "Job qualification, tech assignment, capacity rules, board sync." }],
    features: [{ name: "Job qualification scoring" }, { name: "Capacity protection" }, { name: "Member and warranty tags" }, { name: "Dispatch board sync" }, { name: "Human dispatcher handoff" }],
    faqs: [{ question: "Does ROUTE integrate with field service software?", answer: "Yes — API, Zapier, and direct integrations where available. We map your dispatch rules in onboarding." }],
  },
  "ai-voice-sales": {
    title: "CLOSE — AI Phone Sales Malta | OARC Digital",
    description: "CLOSE answers pricing calls, sells memberships and packages, sends agreements, and runs outbound follow-up — speed-to-lead in seconds.",
    serviceType: "AI Sales Agent — Voice",
    offers: [{ name: "CLOSE Sales Voice", priceFrom: 1190, unitText: "MONTH", description: "Inbound pricing, package sales, agreement send, outbound cadence." }],
    features: [{ name: "Inbound pricing calls" }, { name: "Membership and plan sales" }, { name: "Agreement delivery" }, { name: "Outbound lead follow-up" }, { name: "CRM logging" }],
    faqs: [{ question: "Can CLOSE send service agreements?", answer: "Yes — within rules you set for deposits, instalments, and approval gates configured in onboarding." }],
  },
  "ai-voice-follow-up": {
    title: "RETURN — AI Follow-up & Outbound Malta | OARC Digital",
    description: "RETURN runs confirmation calls, no-show recovery, happy calls, and lapsed customer win-back — voice + SMS cadences you control.",
    serviceType: "AI Outbound Voice & SMS — Follow-up",
    offers: [{ name: "RETURN Outbound", priceFrom: 790, unitText: "MONTH", description: "Confirmations, no-show recovery, win-back campaigns, dashboard control." }],
    features: [{ name: "Pre-visit confirmations" }, { name: "No-show recovery" }, { name: "Happy-call feedback" }, { name: "Lapsed customer win-back" }, { name: "Lead chase cadences" }],
    faqs: [{ question: "Is outbound calling GDPR-compliant?", answer: "Configured with consent flows and quiet hours you set. We advise on Malta GDPR and marketing consent during onboarding." }],
  },
};
