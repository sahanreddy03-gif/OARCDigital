...
export async function generateMetadata({ params }: { params: { location: string; slug: string; service: string } }): Promise<Metadata> {
  const loc = locationNames[params.location];
  const ind = industryData[params.slug];
  ...
  const canonical = `https://oarcdigital.com/malta/${params.location}/${params.slug}/${params.service}`;
  return {
    title,
    description,
...
export default function IndustryLocationServicePage({ params }: { params: { location: string; slug: string; service: string } }) {
  const { location, slug: industry, service } = params;
