...
import { VoiceTalkCTA, VoiceFounderWedge, VoiceShadowTraining, VoiceWorkflowScore } from './VoiceCinemaSections';
...
export default function VoiceProductPage({ config }: { config: StandaloneProductConfig }) {
  ...
  return (
    <H360ProductShell eyebrow={config.eyebrow} h1={config.h1} live={config.live} ctaName={config.ctaName} themeAccent={theme.accent} cinema>
      <section style={{ position: 'relative', overflow: 'hidden', background: theme.gradient, borderBottom: `1px solid ${C.border}`, marginTop: -8, minHeight: 'min(88vh, 860px)' }}>
        <AiAgentsHeroVideo overlay="bottom" />
        <div style={{ position: 'absolute', inset: 0, background: theme.glow, pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(4,18,8,0.4) 0%, rgba(4,18,8,0.92) 75%)', pointerEvents: 'none' }} />
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '48px 24px 56px', position: 'relative', zIndex: 2 }}>
          <div style={{ marginBottom: 32, maxWidth: 560 }}>
            <m.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'inline-flex', alignItems: 'baseline', gap: 12, marginBottom: 20, padding: '14px 18px', borderRadius: 16, background: 'rgba(0,0,0,0.35)', border: `1px solid ${G.greenMid}66` }}>
            ...
            <h2 style={{ fontSize: 'clamp(24px, 3.5vw, 32px)', fontWeight: 800, lineHeight: 1.15, color: 'rgba(255,255,255,0.95)', margin: '0 0 12px', letterSpacing: '-0.03em', fontFamily: FONT_DISPLAY }} data-speakable>
              {config.hero.hook ?? config.hero.ownerPain}
            </h2>
            <p style={{ fontSize: 17, fontWeight: 700, color: G.greenLt, lineHeight: 1.35, margin: '0 0 8px' }}>{config.hero.guestGain}</p>
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 24px 32px' }}>
          <m.blockquote initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} style={{ margin: 0, padding: '16px 20px', borderLeft: `3px solid ${G.greenLt}`, background: 'rgba(0,0,0,0.25)', borderRadius: '0 12px 12px 0', fontSize: 'clamp(14px, 2vw, 16px)', fontWeight: 700, color: 'rgba(255,255,255,0.88)', lineHeight: 1.4, fontFamily: FONT_DISPLAY, maxWidth: 640 }}>
      ...
      <section style={{ background: C.bg, borderTop: `1px solid ${C.border}`, padding: '72px 24px 80px' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}><SignalPulseBoard signals={config.signals} accent={G.greenLt} voicePulse /></div>
      </section>
