...
export default function H360Hero() {
  ...
  return (
    <div style={{ fontFamily:'"Inter",system-ui,-apple-system,Arial,sans-serif', background:G.bg, color:G.text, overflowX:'hidden' }}>
      ...
      {/* ── STATS STRIP ──────────────────────────────────────────────── */}
      <section style={{ padding: isMobile ? '32px 20px' : '48px 60px', borderTop:`1px solid ${G.border}`, background:'#fafafa' }}>
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap:24, maxWidth:860, margin:'0 auto', textAlign:'center' }}>
          {[
            { value:'50+',    label:'Malta restaurants' },
            { value:'+34%',   label:'Avg revenue uplift' },
            { value:'4,200+', label:'Reviews generated' },
            { value:'€2.1M',  label:'Commission saved' },
          ].map(s=>(
            <div key={s.label}>
              <div style={{ fontSize: isMobile ? 28 : 34, fontWeight:800, color:G.green, letterSpacing:'-0.03em', lineHeight:1 }}>{s.value}</div>
              <div style={{ fontSize:13, color:G.textMuted, marginTop:4 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── CASE STUDIES ─────────────────────────────────────────────── */}
      <section style={{ padding: isMobile ? '48px 20px' : '80px 60px', background:G.bg, borderTop:`1px solid ${G.border}` }}>
        <h2 style={{ fontSize: isMobile ? 30 : 40, fontWeight:800, letterSpacing:'-0.03em', color:G.text, textAlign:'center', marginBottom:8 }}>
          Grow sales like these owners
        </h2>
        <p style={{ textAlign:'center', color:G.textMuted, fontSize:15, marginBottom:40 }}>Real Malta restaurants. Real results.</p>
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap:16, maxWidth:980, margin:'0 auto' }}>
          {[
            ...
            <div key={c.name} style={{ borderRadius:14, overflow:'hidden', border:`1px solid ${G.border}`, background:G.bg }}>
              <div style={{ height:160, background:`linear-gradient(135deg,${G.green},${G.greenMid})`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:48 }}>{c.photo}</div>
              <div style={{ padding:'18px 20px' }}>
                <div style={{ fontSize:30, fontWeight:800, color:G.green, letterSpacing:'-0.03em', lineHeight:1 }}>{c.stat}</div>
                <div style={{ fontSize:12, color:G.textMuted, margin:'3px 0 12px' }}>{c.label}</div>
      ...
      {/* ── FINAL CTA ────────────────────────────────────────────────── */}
      <section style={{ padding: isMobile ? '60px 20px' : '80px 40px', background:G.green, textAlign:'center' }}>
        <h2 style={{ fontSize: isMobile ? 28 : 42, fontWeight:800, letterSpacing:'-0.03em', color:'#fff', marginBottom:12 }}>
          Ready to own your market?
        </h2>
        <p style={{ fontSize: isMobile ? 15 : 17, color:G.greenLt, maxWidth:440, margin:'0 auto 32px', lineHeight:1.55 }}>
          Get your free H360 audit powered by ARC AI. See exactly what's costing you money — in 60 seconds.
        </p>
        <div style={{ display:'inline-flex', alignItems:'center', background:G.bg, border:`1.5px solid ${G.border}`, borderRadius:14, padding:'6px 6px 6px 16px', boxShadow:'0 2px 24px rgba(0,0,0,0.18)', gap:8, width:'100%', maxWidth:480 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, marginTop:12, fontSize:12, color:G.greenLt, opacity:0.8 }}>
