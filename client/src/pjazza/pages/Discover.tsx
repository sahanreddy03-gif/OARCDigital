      ...
      <div className="pj-section-tight" style={{ paddingTop: 0 }}>
        ...
        >
          ...
          <p style={{ fontSize: 'var(--pj-size-h3)', fontWeight: 700, color: 'var(--pj-text)', lineHeight: 1.4, marginBottom: 12 }}>
            Noni's Kitchen went live for 20 minutes.
            <br />
            <span style={{ color: 'var(--pj-gold)' }}>47 viewers. 12 bookings. €540 revenue.</span>
          </p>
          <p style={{ fontSize: 'var(--pj-size-xs)', color: 'var(--pj-text-tertiary)' }}>
            Sliema · Last Tuesday · Restaurant
          </p>
        </div>
      </div>
    </ScrollReveal>
  ...
  return (
    <ScrollReveal>
      <div className="pj-section-tight">
        <h2 style={{ fontSize: 'var(--pj-size-h2)', fontWeight: 700, color: 'var(--pj-text)', marginBottom: 16, letterSpacing: '-0.01em' }}>
          Browse by category
        </h2>
  ...
  const steps = [
    {
      ...
      desc: 'Watch real businesses in action across Malta. See the food, the view, the product — live.',
    },
    {
      num: '02',
      title: 'Talk to the seller',
      desc: 'Ask questions in real time. Request a closer look. Negotiate directly.',
    },
    {
      num: '03',
      title: 'Buy with escrow protection',
      desc: 'Your money is held safely until you confirm. Every transaction protected.',
    },
  ];
        ...
        {steps.map((step, i) => (
          <ScrollReveal key={i} delay={i * 80}>
            ...
            >
              ...
              <div>
                <h3 style={{ fontSize: 'var(--pj-size-body)', fontWeight: 700, color: 'var(--pj-text)', marginBottom: 4 }}>
                  {step.title}
                </h3>
                <p style={{ fontSize: 'var(--pj-size-small)', color: 'var(--pj-text-tertiary)', lineHeight: 1.5 }}>
                  {step.desc}
                </p>
              </div>
            </div>
...
function TrustSection() {
  const features = [
    { Icon: Shield, title: 'Escrow Protection', desc: 'Money held safely until you confirm delivery' },
        ...
        {features.map((f, i) => (
          <ScrollReveal key={i} delay={i * 80}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', padding: '12px 0' }}>
              ...
              <div>
                <h3 style={{ fontSize: 'var(--pj-size-body)', fontWeight: 700, color: 'var(--pj-text)', marginBottom: 2 }}>
                  {f.title}
                </h3>
                <p style={{ fontSize: 'var(--pj-size-small)', color: 'var(--pj-text-tertiary)', lineHeight: 1.5 }}>
                  {f.desc}
...
function HighValueSection() {
  const items = [
    { title: 'Sea View 2-Bed Apartment', location: 'Sliema', price: '€1,350/mo', img: thumbProperty, badge: 'Live Tour' },
    { title: '40ft Catamaran Charter', location: 'Grand Harbour', price: '€1,200/day', img: thumbYacht, badge: 'Live Tour' },
    { title: '2024 VW Polo 1.0 TSI', location: '12,000 km', price: '€18,500', img: thumbCar, badge: 'Walkaround' },
  ];
            ...
            <div className="pj-card pj-touch" style={{ overflow: 'hidden' }} data-testid={`card-listing-${i}`}>
              <div style={{ display: 'flex' }}>
                ...
                <div style={{ padding: 14, flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h3 style={{ fontSize: 'var(--pj-size-small)', fontWeight: 700, color: 'var(--pj-text)', marginBottom: 4 }}>
                    {item.title}
                  </h3>
                  <p style={{ fontSize: 'var(--pj-size-micro)', color: 'var(--pj-text-tertiary)', marginBottom: 8 }}>
                    {item.location}
                  ...
                  <span
                    className="pj-mono"
                    style={{ fontSize: 'var(--pj-size-h3)', fontWeight: 800, color: 'var(--pj-text)' }}
                  >
                    {item.price}
                  </span>
                </div>
          ...
          <ScrollReveal key={i} delay={i * 60}>
            <div className="pj-card" style={{ padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                ...
                <span className="pj-mono" style={{ fontSize: 'var(--pj-size-h3)', fontWeight: 800, color: v.color }}>
                  {v.percent}%
                </span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 'var(--pj-size-xs)', fontWeight: 600, color: 'var(--pj-text-secondary)' }}>
                  {v.status}
                </span>
