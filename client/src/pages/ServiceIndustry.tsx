// Programmatic Service × Industry Pages
// SEO: [Service] for [Industry] in Malta | OARC Digital
...
const industryMap: Record<string, IndustryMeta> = {
...
// Specific combo overrides for highest-value combinations
...
function getComboContent(serviceSlug: string, industry: string, serviceMeta: ServiceMeta, industryMeta: IndustryMeta): ComboContent {
  const key = `${serviceSlug}_${industry}`;
  if (comboOverrides[key]) return comboOverrides[key];

  // Generic fallback with service+industry specificity
  return {
...
export default function ServiceIndustry() {
  const [, params] = useRoute('/services/:serviceSlug/:industry');
  const serviceSlug = params?.serviceSlug;
  const industry = params?.industry;

  if (!serviceSlug || !industry || !serviceMap[serviceSlug] || !industryMap[industry]) {
    return (
      <Layout>
  ...
  const seoTitle = `${service.title} for ${ind.plural} in Malta | OARC Digital`;
  const seoDescription = `Professional ${service.title.toLowerCase()} for ${ind.plural.toLowerCase()} in Malta. OARC Digital delivers measurable results. Book a free strategy call today.`;
  ...
  return (
    <Layout>
      <SEOHead
        ...
        canonicalUrl={`https://oarcdigital.com/services/${serviceSlug}/${industry}`}
      />
