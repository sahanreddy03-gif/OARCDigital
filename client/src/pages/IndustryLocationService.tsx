...
export default function IndustryLocationService() {
  const [, params] = useRoute('/malta/:location/:industry/:service');
  ...
  if (!loc || !ind || !svc) return <NotFound />;

  const pageTitle = `${svc.name} for ${ind.plural} in ${loc.name}, Malta | OARC Digital`;
  const pageDesc = `OARC Digital provides ${svc.name.toLowerCase()} for ${ind.plural} in ${loc.name}. ${ind.painPoint.charAt(0).toUpperCase() + ind.painPoint.slice(1)} — we solve it.`;
  const canonicalUrl = `https://oarcdigital.com/malta/${location}/${industry}/${service}`;
