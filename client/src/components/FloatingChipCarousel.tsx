...
export default function FloatingChipCarousel() {
  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="flex animate-scroll whitespace-nowrap gap-2">
        {duplicatedServices.map((service, index) => (
          <div 
            key={index} 
            className="inline-flex flex-shrink-0"
          >
            <div className="group flex items-center gap-2 px-2.5 md:px-3 py-1.5 md:py-2 bg-white/95 backdrop-blur-sm rounded-full shadow-lg hover:shadow-xl transition-all cursor-pointer border border-white/20">
              <div className="w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-md overflow-hidden flex-shrink-0">
                <img 
                  src={service.image} 
                  alt={service.text}
              ...
              <span className="text-[10px] md:text-xs lg:text-sm font-bold text-gray-900 pr-1 whitespace-nowrap">
                {service.text}
              </span>
            </div>
          </div>
