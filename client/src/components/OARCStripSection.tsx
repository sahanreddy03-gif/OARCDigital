  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 md:px-6 max-w-5xl">
        ...
        {/* OARC Pills Row */}
        <div className="flex justify-center items-center gap-3 md:gap-6 lg:gap-8">
          {oarcItems.map((item, index) => (
            <div
              key={item.letter}
              className={`flex items-center gap-1.5 md:gap-2 px-3 md:px-5 py-2 md:py-2.5 rounded-full bg-zinc-50 border border-zinc-100 transition-all duration-700 ${
                isVisible
            ...
            >
              {/* Letter - Green accent */}
              <span className="text-lg md:text-xl lg:text-2xl font-bold text-[#c4ff4d] drop-shadow-sm">
                {item.letter}
              </span>
              {/* Word */}
              <span className="text-xs md:text-sm lg:text-base font-medium text-zinc-700 whitespace-nowrap">
                {item.word}
              </span>
            </div>
          ))}
