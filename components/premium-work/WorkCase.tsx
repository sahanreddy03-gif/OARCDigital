...
function LiveBrandRecord({ record, isClient }: { record: LiveRecord; isClient?: boolean }) {
  ...
  return <section className="live-brand-record" aria-labelledby="live-brand-title"><div><p>{record.confidential ? "PRIVATE BRAND RECORD" : isClient ? "LIVE BRAND RECORD" : "LIVE PRODUCT RECORD"}</p><h2 id="live-brand-title">{title}</h2><p className="live-brand-relationship">{record.relationship}</p><p className="live-brand-verified">{record.verified}</p></div><div className="live-brand-actions">{record.links.map((link) => <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer"><span>{link.note}</span><b>{link.label}</b><ArrowUpRight size={18} /></a>)}<div className="instagram-status"><b>{record.confidential ? "PUBLIC LINKS" : "INSTAGRAM"}</b><p>{record.instagramStatus}</p></div></div></section>;
}

function OriginalStudyRecord({ story }: { story: Story }) {
  return <section className="original-study-record" aria-labelledby="original-study-record-title"><div><p>OARC ORIGINAL STUDY</p><h2 id="original-study-record-title">A designed direction,<br /><i>made tangible.</i></h2></div><div><p>{story.mode}</p><p>This studio-led scenario uses narrative, art direction, and illustrative artefacts to make a business or operating-system idea tangible. The value is in the point of view, the system, and the way the experience is shaped.</p><strong>Studio direction, presented with the same care as every OARC record.</strong></div></section>;
}

function ConfidentialRecord({ story }: { story: Story }) {
  return <section className="confidential-record" aria-labelledby="confidential-record-title"><div><p>PRIVATE WORK</p><h2 id="confidential-record-title">The work stays<br /><i>considered.</i></h2></div><div><p>{story.mode}</p><p>This record keeps the client and private systems off the page while making the design and operating principle clear. The value is in the decisions, structure, and experience made visible here.</p><strong>Selected detail, shown at the level approved for this collection.</strong></div></section>;
}
...
function OriginalChapters({ story }: { story: Story }) {
  ...
  return <section className={`original-journey journey-${structure} original-family-${family}`} aria-label={`${story.name} original study journey`}><header><p>THE ORIGINAL JOURNEY</p><h2>{story.name.split(" ").slice(0, 2).join(" ")}<br /><i>in three decisive moves.</i></h2><span>Pressure, intervention, and intended value—kept clear from first frame to final move.</span></header><div className="original-journey-beats">{story.chapters.map((chapter, index) => <article key={chapter.number} data-beat={index + 1}><div className="original-chapter-key"><b>{chapter.number}</b><span>{chapter.label}</span></div><h3>{chapter.title}</h3><p>{chapter.body}</p><em>{instruction}</em></article>)}</div></section>;
}
...
function CaseProofLedger({ story, slug, isOriginal }: { story: Story; slug: string; isOriginal: boolean }) {
  const clientLedger = { state: "CLIENT WORK / SELECTED CASE", role: "A named client or event brief, its working scope, and the selected visual direction are brought together in one case.", check: "The public destination, the work shown here, and the record’s own context.", source: `${story.sourceLabel}.`, reviewed: "Selected work / 2026" };
  const ledger = PROOF_LEDGERS[slug] ?? (story.clientRecord ? clientLedger : { state: "OARC WORK / METHOD RECORD", role: "OARC developed this direction to make a specific business or operating problem tangible and easier to act on.", check: "The narrative, designed artefacts, and working logic presented on this page.", source: "OARC method record.", reviewed: "OARC work / 2026" });
...
function CaseAnswerPanel({ story }: { story: Story }) {
  ...
  return <section className="case-answer-panel" aria-labelledby="case-answer-title"><div><p>THE WORK IN ONE LINE</p><h2 id="case-answer-title">What did OARC<br /><i>make possible?</i></h2></div><dl><div><dt>THE WORK</dt><dd>{story.liveRecord.relationship}</dd></div><div><dt>THE VISIBLE SHIFT</dt><dd>{story.facts.join(" · ")}</dd></div><div><dt>THE PUBLIC RECORD</dt><dd><a href={story.source} target="_blank" rel="noopener noreferrer">{story.sourceLabel} <ArrowUpRight size={14} /></a></dd></div></dl></section>;
}
...
function CaseDiscoveryDepth({ content, isOriginal, isConfidential }: { content: DiscoveryContent; isOriginal: boolean; isConfidential: boolean }) {
  const label = isOriginal ? "OARC WORK / METHOD" : isConfidential ? "RESTRICTED WORK / PUBLIC VIEW" : "PUBLIC PRODUCT / WORK";
...
export default function WorkCase({ slug = "pjazza" }: { slug?: string }) {
  ...
  return <main className={`case-page documentary case-${slug} case-format-${caseFormat} ${isOriginal ? `original-case structure-${story.structure}` : ""} ${isClient ? "client-case" : ""}`} style={rootStyle}>
    <header className="case-nav-light"><Link href="/new-work" className="oarc-brand light-brand" aria-label="OARC Digital Work"><span className="brand-word"><b>OARC</b><i>DIGITAL</i></span></Link><span className="case-signal">OARC SIGNAL / {isOriginal ? "ORIGINAL" : isConfidential ? "RESTRICTED" : isClient ? "CLIENT WORK" : "PUBLIC RECORD"}</span><Link href="/new-work" className="back-link-light"><ChevronLeft size={16} /> All stories</Link></header>
    ...
    <section className="source-disclosure"><span>{isOriginal ? "STUDIO DIRECTION" : "THE CLOSING FRAME"}</span><h2>{story.closeTitle}</h2><p>{story.closeText}</p>{isOriginal ? <span className="source-static-label">ORIGINAL OARC DIRECTION / STUDIO SCENARIO</span> : <a href={story.source} target="_blank" rel="noopener noreferrer">Visit the approved public source <ArrowUpRight size={17} /></a>}</section>
