...
type Filter = "All client work" | "Client partnerships" | "Client products" | "Private client work" | "Client concept work";
...
const clientProjects = Object.entries(CLIENT_CASE_STUDIES).map(([slug, story], index) => ({
  ...
  title: story.name,
  line: story.intro,
  evidence: CLIENT_SCOPE[slug] ?? "OARC scope and approved brand links inside",
  image: story.image,
...
const labProjects = Object.entries(ORIGINAL_STUDIES)
  ...
  .map(([slug, story], index) => ({
    ...
    evidence: "Client project · method and prototype inside",
    image: story.image,
...
const filters: Filter[] = ["Client partnerships", "Client products", "Private client work", "Client concept work", "All client work"];
...
export default function NewWorkIndex() {
  ...
  return <main className="work-index work-index-light" id="new-work-top">
    <section className="light-hero">
      ...
      <div className="light-hero-copy"><p className="ink-label">CLIENT PARTNERSHIPS · CLIENT PRODUCTS · PRIVATE CLIENT WORK</p><h1>WORK YOU CAN<br /><i>TRUST.</i></h1><div><p><strong>Real brands and real OARC delivery lead this collection.</strong> Client products, private client work, and concept-led client projects follow in clearly marked sections—so every record says exactly what it is.</p><a href="#new-work-records" aria-label="View selected OARC work"><ArrowDownRight size={23} /></a></div></div>
      <div className="hero-collage"><figure className="hero-collage-a"><img src={clientProjects[0]?.image} alt={clientProjects[0]?.alt} decoding="async" fetchPriority="high" /><figcaption>FEATURED CLIENT PARTNERSHIP</figcaption></figure><figure className="hero-collage-b"><img src={labProjects[0]?.image} alt={labProjects[0]?.alt} loading="eager" decoding="async" /><figcaption>FEATURED CLIENT CONCEPT PROJECT</figcaption></figure><span>NEW<br />WORK<br />2026</span></div>
    </section>

    <section className="work-answer-index" aria-labelledby="new-work-answer-title"><div><p>THE DIRECT ANSWER</p><h2 id="new-work-answer-title">What will you<br /><i>find here?</i></h2></div><p>Client partnerships show the brand and OARC’s delivery. Client products show live product work. Private client projects show the business problem and delivery approach. Concept-led client projects show the system and prototype work behind a focused brief.</p><a href="#new-work-standard">How to read the collection <ArrowUpRight size={17} /></a></section>
